...
export class ShapeElement extends Element {
  ...
  constructor(data: ElementData, inFrame: number, outFrame: number, startTime: number) {
    super();
    if (!data) return;
    this.name = data.nm;
    this.type = data.ty;
  ...
  setupShapeByType(data: ElementData | PathData | StrokeData | TrimData | RectData | EllipseData | FillData | TransformData) {
    switch (data.ty) {
      case 'sh':
  ...
  static createAnimatePos(animData: Animation, frame: number, fromPos: PIXI.Point, toPos: PIXI.Point) {
  ...
  drawTrim(frame: number) {
    ...
    if (frame < startAnim[0].startFrame && frame < endAnim[0].startFrame) return;
